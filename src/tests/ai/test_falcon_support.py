...
# -*- coding: utf-8 -*-
"""
Unit tests for the falcon_support module.

This module contains tests for the Falcon model support functionality,
...
class TestFalconSupport(BaseTest):
    """Unit tests for the falcon_support module."""
    ...
    def setUp(self):
        ...
        # Skip tests if Falcon models are not available
        if not is_falcon_available():
            self.skipTest("Falcon models are not available")
    ...
    def test_is_falcon_available(self):
        """Test checking if Falcon models are available."""
        # This test will always pass since we skip tests if Falcon models are not available
        self.assertTrue(is_falcon_available())
    
    def test_check_dependencies(self):
        """Test checking dependencies for Falcon models."""
        ...
        # All dependencies should be True since we skip tests if Falcon models are not available
        for dep, available in dependencies.items():
            self.assertTrue(available, f"Dependency {dep} should be available")
    
    def test_get_available_models(self):
        """Test getting available Falcon models."""
    ...
    @patch("src.ai.falcon_support.AutoModelForCausalLM")
    @patch("src.ai.falcon_support.AutoTokenizer")
    async def test_load_model(self, mock_tokenizer, mock_model):
        """Test loading a Falcon model."""
        # Set up mocks
        ...
        self.assertIn("tiiuae/falcon-7b-instruct_cpu_None", MODEL_CACHE)
    
    @patch("src.ai.falcon_support.AutoModelForCausalLM")
    @patch("src.ai.falcon_support.AutoTokenizer")
    async def test_load_model_with_cache(self, mock_tokenizer, mock_model):
        """Test loading a Falcon model with caching."""
        # Set up mocks
        mock_model_instance = MagicMock()
        mock_model.from_pretrained.return_value = mock_model_instance
        mock_tokenizer_instance = MagicMock()
    ...
    async def test_load_model_without_cache(self, mock_tokenizer, mock_model):
        """Test loading a Falcon model without using cache."""
        # Set up mocks
        mock_model_instance = MagicMock()
        mock_model.from_pretrained.return_value = mock_model_instance
        mock_tokenizer_instance = MagicMock()
    ...
    @patch("src.ai.falcon_support.MODEL_CACHE", {
        "tiiuae/falcon-7b-instruct_cpu_None": ("model1", "tokenizer1"),
        "tiiuae/falcon-7b_cpu_None": ("model2", "tokenizer2")
    })
    async def test_unload_model(self):
        """Test unloading a Falcon model."""
        # Unload model
        ...
        self.assertNotIn("tiiuae/falcon-7b-instruct_cpu_None", MODEL_CACHE)
        self.assertIn("tiiuae/falcon-7b_cpu_None", MODEL_CACHE)
    ...
    async def test_clear_model_cache(self):
        """Test clearing the Falcon model cache."""
        # Clear cache
    ...
    @patch("src.ai.falcon_support.load_model")
    async def test_generate_text(self, mock_load_model):
        """Test generating text with a Falcon model."""
        # Set up mocks
        ...
        mock_tokenizer.decode.return_value = "User: Test prompt\nAssistant: This is a test response."
    ...
    async def test_generate_text_with_error(self, mock_load_model):
        ...
        with self.assertRaises(FalconInferenceError):
    ...
    async def test_generate_text_streaming(self, mock_load_model):
        ...
        with patch("src.ai.falcon_support.TextIteratorStreamer", return_value=mock_streamer):
            # Set up callback
            callback = MagicMock()
    ...
    @patch("src.ai.falcon_support.generate_text")
    async def test_chat_completion(self, mock_generate_text):
        """Test chat completion with a Falcon model."""
        # Set up mock
        ...
        self.assertIn("User: Tell me a joke.", prompt)
        self.assertIn("Assistant:", prompt)
    
    @patch("src.ai.falcon_support.generate_text_streaming")
    ...
    async def test_optimize_model(self, mock_load_model):
        """Test optimizing a Falcon model."""
        # Set up mocks
        mock_model = MagicMock()
        mock_tokenizer = MagicMock()
        mock_load_model.return_value = (mock_model, mock_tokenizer)
...
# Pytest-style tests
class TestFalconSupportPytest:
    """Pytest-style tests for the falcon_support module."""
    ...
    def setup(self, base_test_fixture):
        ...
        # Skip tests if Falcon models are not available
        if not is_falcon_available():
            pytest.skip("Falcon models are not available")
    ...
    def test_is_falcon_available_pytest(self, setup):
        """Test checking if Falcon models are available using pytest style."""
        # This test will always pass since we skip tests if Falcon models are not available
        assert is_falcon_available()
    
    def test_check_dependencies_pytest(self, setup):
        """Test checking dependencies for Falcon models using pytest style."""
        dependencies = check_dependencies()
    ...
    def test_get_available_models_pytest(self, setup):
        """Test getting available Falcon models using pytest style."""
        models = get_available_models()
    ...
    async def test_load_model_pytest(self, mock_tokenizer, mock_model, setup):
        """Test loading a Falcon model using pytest style."""
        # Set up mocks
        mock_model_instance = MagicMock()
        mock_model.from_pretrained.return_value = mock_model_instance
        mock_tokenizer_instance = MagicMock()
        ...
        # Verify model is in cache
        assert "tiiuae/falcon-7b-instruct_cpu_None" in MODEL_CACHE
    
    @pytest.mark.asyncio
    @patch("src.ai.falcon_support.load_model")
    async def test_generate_text_pytest(self, mock_load_model, setup):
        """Test generating text with a Falcon model using pytest style."""
        # Set up mocks
        mock_model = MagicMock()
        mock_tokenizer = MagicMock()
        mock_load_model.return_value = (mock_model, mock_tokenizer)
    ...
    async def test_chat_completion_pytest(self, mock_generate_text, setup):
        """Test chat completion with a Falcon model using pytest style."""
        # Set up mock
        mock_generate_text.return_value = "This is a test response."
