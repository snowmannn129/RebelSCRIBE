    ...
    with open(document_manager_path, 'r') as src, open(fixed_file_path, 'w') as dst:
        for i, line in enumerate(src):
            if i == 55:  # Line 56 (0-indexed)
                ...
                dst.write('        self.max_versions = self.config.get("documents", "max_versions", self.DEFAULT_MAX_VERSIONS)\n')
                print(f"Replaced with: {'        self.max_versions = self.config.get(\"documents\", \"max_versions\", self.DEFAULT_MAX_VERSIONS)\\n'}")
            else:
                dst.write(line)
